...
<div style="max-width:660px; margin:0 auto; padding:2rem 0; font-family:inherit;">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.75rem;">
        ...
        <div style="text-align:right;">
            <p style="font-size:13px; font-weight:500; color:#111; margin:0 0 1px;">{{ auth()->user()->name }}</p>
            <p style="font-size:12px; color:#bbb; margin:0;">{{ auth()->user()->email }}</p>
        </div>
    </div>
    ...
    <div style="background:#fff; border:0.5px solid #e8e8e8; border-radius:14px; overflow:hidden; margin-bottom:1.5rem; box-shadow:0 1px 3px rgba(0,0,0,.04);" id="admins-list">
        ...
        <a href="{{ route('chat.show', $user) }}"
           ...
           style="display:flex; align-items:center; justify-content:space-between; padding:11px 15px; border-bottom:0.5px solid #f2f2f2; text-decoration:none; background:{{ $hasUnread ? '#fafbff' : 'transparent' }}; transition:background .12s; {{ $loop->last ? 'border-bottom:none;' : '' }}"
           onmouseover="this.style.background='#f7f7f9'" onmouseout="this.style.background='{{ $hasUnread ? '#fafbff' : 'transparent' }}'">
            <div style="display:flex; align-items:center; gap:11px; min-width:0;">
                <div style="width:37px; height:37px; border-radius:50%; background:{{ $avatarBg }}; color:{{ $avatarColor }}; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:600; flex-shrink:0;">
                    {{ strtoupper(substr($user->name, 0, 1)) }}
                </div>
                <div style="min-width:0;">
                    <div style="display:flex; align-items:center; gap:5px; margin-bottom:2px;">
                        <p class="user-name" style="font-size:13px; font-weight:{{ $hasUnread ? '600' : '500' }}; color:#111; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $user->name }}</p>
                        <span style="font-size:10px; font-weight:500; padding:1px 6px; border-radius:99px; background:{{ $avatarBg }}; color:{{ $avatarColor }}; flex-shrink:0;">{{ $isSuperAdmin ? 'Superadmin' : 'Admin' }}</span>
                    </div>
                    <p class="user-preview" style="font-size:12px; color:{{ $hasUnread ? '#444' : '#bbb' }}; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:310px; font-weight:{{ $hasUnread ? '500' : '400' }};">
                        {{ $lastMsg ? mb_strimwidth($lastMsg->body, 0, 80, '…') : 'No messages yet' }}
                    </p>
                </div>
            </div>
            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:5px; flex-shrink:0; margin-left:10px;">
                <span class="user-time" style="font-size:11px; color:#ccc; white-space:nowrap;">{{ $lastMsg ? $lastMsg->created_at->diffForHumans(null, true, true) : '' }}</span>
                @if($hasUnread)
                <span class="unread-badge" style="font-size:11px; font-weight:600; background:#1d4ed8; color:#fff; border-radius:99px; padding:1px 7px; min-width:20px; text-align:center;">{{ $unread }}</span>
                @else
           ...
           onmouseover="this.style.background='#f7f7f9'" onmouseout="this.style.background='{{ $hasUnread ? '#fafbff' : 'transparent' }}'">
            <div style="display:flex; align-items:center; gap:11px; min-width:0;">
                <div style="position:relative; flex-shrink:0;">
                    ...
                    <span class="avatar-dot" style="position:absolute; top:0; right:0; width:8px; height:8px; background:#1d4ed8; border-radius:50%; border:2px solid #fff; {{ $hasUnread ? '' : 'display:none;' }}"></span>
                </div>
                <div style="min-width:0;">
                    <p class="user-name" style="font-size:13px; font-weight:{{ $hasUnread ? '600' : '500' }}; color:#111; margin:0 0 2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $user->name }}</p>
                    <p class="user-preview" style="font-size:12px; color:{{ $hasUnread ? '#444' : '#bbb' }}; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:320px; font-weight:{{ $hasUnread ? '500' : '400' }};">
                        {{ $lastMsg ? mb_strimwidth($lastMsg->body, 0, 80, '…') : 'No messages yet' }}
                    </p>
                </div>
            </div>
